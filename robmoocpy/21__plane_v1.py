...
# On itère sur les données enregistrées avec un pas (ex: 10) pour fluidifier
...
# # On itère sur les données enregistrées avec un pas (ex: 50) pour fluidifier
# for i in range(0, len(x_history), 50):
#     ax.cla() # Efface explicitement les anciens tracés de la mémoire
...
#     ax.view_init(elev=90, azim=-90) # On remet la vue de dessus après avoir effacé/redessiné le graphe
#     pause(0.01)