...
Vd = np.linspace(10,vd,m) # speed profile for the line following controller
print("Vd=",Vd)
        ...
        if MJ[i] >= jmax:
            ...
            if np.dot(ab.T,mb) < 0:
                ...
                if (i<m-1) and (MJ[i] == MJ[i+1]): # wait a robot i to finish a line to start robot i+1
                    ...
                    u1= a1*tanh(vd-X[3,0])-tanh((m-i)/m)#*(m-i)/m
                    # u1= a1*tanh(vd-max(X[3,0]-i*vd,0))#-tanh((m-i)/m)#*(m-i)/m
                    u1= a1*tanh(Vd[i]-X[3,0]) #-tanh((m-i)/m)#*(m-i)/m
